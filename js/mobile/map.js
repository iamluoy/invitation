var map = {
	...
	mapShow : function(option) {
		...
		// 地图添加
		var detal = option.detal, latitude = option.latitude, longitude = option.longitude, fnOpen = option.fnOpen, fnClose = option.fnClose;
		...
		if ($(this).attr('data-mapIndex') != __map._mapIndex) {
			__map._map
					.html($('<div class="bk"><span class="css_sprite01 s-bg-map-logo"></span></div>'));
		...
		setTimeout(function() {
			...
			if (__map._map.find('div').length >= 1) {
				...
				setTimeout(function() {
					...
					if (!__map._mapValue)
						__map.addMap(detal, latitude, longitude, fnOpen,
								fnClose);
				}, 500)
			} else
				return;
	...
	addMap : function(detal, latitude, longitude, fnOpen, fnClose) {
		var detal = detal, latitude = Number(latitude), longitude = Number(longitude);

		var fnOpen = typeof (fnOpen) === 'function' ? fnOpen : '', fnClose = typeof (fnClose) === 'function' ? fnClose
				: '';
