function Lottery(node,cover, coverType, width, height, percent,drawPercentCallback) {
	// canvas
	this.conNode = node;
...
Lottery.prototype = {
	...
	bindEvent : function() {
		var _this = this;
		// 判断是否为移动端
		var device = (/android|webos|iphone|ipad|ipod|blackberry|iemobile|opera mini/i
	...
	drawLottery : function() {
		if (this.lotteryType == 'image') {
			...
			this.backCtx.fillText(this.lottery, this.width / 2, this.height / 2
					+ fontSize / 2);
			this.backCtx.restore();
			this.drawMask();
		}
	...
	drawMask : function() {
		if (this.coverType == 'color') {
			...
			image.onload = function() {
				...
				// _this.maskCtx.drawImage(this, 0, 0,_this.width,
				// _this.height);
				//自适应
				var posX,posY,cutW,cutH,sW,sH;
				if(_this.height/_this.width>this.height/this.width){
					posY=0;
					posX=0-(this.height*_this.width/_this.height-_this.width)/2;
					cutH=this.height;
					cutW=this.height*_this.width/_this.height;
				}else{
					posX=0;
					posY=(_this.width*this.height/this.width-_this.height)/2;
					cutW=this.width;
					cutH=this.width*_this.height/_this.width;
				}
				
				_this.maskCtx.drawImage(this, posX, posY,cutW, cutH, 0,
						0, _this.width, _this.height);
				
				maskinit=true;
				if(window.loadingStep==2 || vcount==0){
					loading.hide();
				...
				var fontSize = 50;
				var txt = '';
				var gradient = _this.maskCtx.createLinearGradient(0, 0,
						_this.width, 0);
				...
				_this.maskCtx.fillText(txt, _this.width / 2
						- _this.maskCtx.measureText(txt).width / 2, 100);
