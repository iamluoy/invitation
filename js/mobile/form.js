		...
		inputs
				.each(function() {
					if (this.name != '' && this.name != 'undefined') {
						...
						var empty_tip = backData.empty_tip, reg = backData.reg, reg_tip = backData.reg_tip;

						// 根据结果处理
						if ($.trim($(this).val()) == '') {
							__form_input.showCheckMessage(note, empty_tip,
									false);
							// $(this).focus();
						...
						if (reg != undefined && reg != '') {
							if (!$(this).val().match(reg)) {
								// $(this).focus();
								$(this).addClass('z-error');
								__form_input.showCheckMessage(note, reg_tip,
										false);
								valid = false;
								return false;
							}
	...
	signUpCheck_submit : function(form, note) {
		...
		$.ajax({
			...
			success : function(msg) {
				...
				if (msg.code == 200) {
					// 提示成功
					__form_input.showCheckMessage($('.u-note'), $(
							'.u-note-sucess').data('type'), true)
