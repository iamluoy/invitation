;(function($) {
    $.fn.yl3d = function(options) {
    	...
    	return this.each(function(){
                ...
                e.preventDefault();
 				if (startPoint_x) {
					 ...
					 if(moveStart_3d){
						 if(param.stopEle){
							//true为垂直,false为水平
 					 		Math.abs(pageY-movePoint_y)>=Math.abs(pageX-movePoint_x)+13 ? position_3d = true : position_3d = false ; 
						 }else{
							position_3d = false; 
						 }
						moveStart_3d = false;
					 ...
					 if(!position_3d){						 			 
						 if(Math.abs(movePoint_x - pageX) >= speed) {
							...
							box.find('img').eq(currentImage-1).css("display","none"); 
							 ...
							 movePoint_x = pageX;
							 var $allImg= box.find('img');
							 $allImg.eq(currentImage-1).css("display","inline");
							 if(currentImage==3 || currentImage==($allImg.length-1)){
								 $allImg.each(function(){
									 var $self=$(this);
									 if($self.attr('src')!=$self.attr('data-bk')){
										 $self.attr('src',$self.attr('data-bk'));										 
									 }
								 });
							 }
						 }
			...
			box.on('mouseup touchend mouseout', function(e) {
				//初始化状态值
 				startPoint_x = null;
				startPoint_y = null;
				movePoint_y	= null;
            ...
            function imginit(){
                ...
                for(var i=$bk.length-1;i>$bk.length-6;i--){
                	var $self=$bk.eq(i);
                	if(!$self || $self.length==0){
                		break;
                	}
